...
altADD= data_tib_m1 %>%  mutate(Contact= case_when(!is.na(D_857915436_D_907038282) | !is.na(D_857915436_D_970839481) | 
                                                     !is.na(D_857915436_D_379!is.na(D_857915436_899229) | !is.na(D_857915436_D_728926441) | 
                                                              !is.na(D_857915436_D_284580415))~"Gave Alternative Address Component(s)",
                                                           TRUE~"Skipped Alternative Address Question"))

dt_altADD_summary <- altADD %>% dplyr::group_by(Contact) %>%  dplyr::summarize(n=n(), percentage=100*n/nrow(.))  %>% dplyr::ungroup() %>%  dplyr::select(Contact, n, percentage)


dt_altADD_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
  tab_header(title = md("ALTADDRESS1: Alternative Address")) %>% 
