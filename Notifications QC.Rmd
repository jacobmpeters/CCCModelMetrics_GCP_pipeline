...
if (use_test_box_folder) {
  boxfolder <- 249228373485 # test box folder
} else {
  boxfolder <- 254808757764 # destination of CSV files (not pdf)
}
...
\FloatBarrier
## The same participant received multiple Verified, BL Survey reminders on the same day. This includes the category 'Verified, BL Survey Reminders New Pts'
```{r VBLSR, echo=FALSE, message=FALSE, warning=FALSE}
VBLSR <- "SELECT distinct(p.Connect_ID), #distinct(n.token), 
n.token,
category,
       DATE(notification_time) as repeat_date, 
       COUNT(*) as num_same_day_attempts
...
VBLSR_table <- bq_project_query(project, VBLSR)
Ver_BLSurv_rem <- bq_table_download(VBLSR_table, bigint = "integer64")


if (nrow(Ver_BLSurv_rem) > 50) {
  cat("There are", nrow(Ver_BLSurv_rem), "errors. This table has been made a csv and posted to Box, link in the report intro.")
  write.csv(Ver_BLSurv_rem,glue("{local_drive}Multiple_Ver_BLSurv_rem_reminders_{currentDate}_boxfolder_{boxfolder}.csv"),row.names = F,na="")
} else {
  if(nrow(Ver_BLSurv_rem) == 0) {
  cat("No errors\n")
} else {
  knitr::kable(Ver_BLSurv_rem) %>% kable_styling(latex_options = "scale_down")
}
}
```
...
# 3. Number of messages per day this past week, compared to the specID daily averages since March 1,2024.
