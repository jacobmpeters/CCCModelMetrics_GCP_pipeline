...
Week_Without_Notification <- "-- Step 1: Filter notifications within the desired date range
...
-- Step 2: Define all possible notifications (excluding specific categories)
AllPossibleNotifications AS (
    ...
    WHERE 
        ...
        category NOT LIKE '% PI Change' and category NOT LIKE 'Prev. Blocked %'
    UNION ALL
    SELECT 
...
NotYetSentNotifications AS (
    ...
    WHERE 
        ...
        AND NOT (apn.category='Baseline Clinical Biospecimen Survey Reminder' OR
                 apn.category='Biospecimen Home Collection Kit Reminder' OR
                 apn.category='Biospecimen Home Collection Survey Reminder' OR
                 apn.category='Biospecimen Survey Reminder' OR
                 apn.category='Biospecimen Home Collection Acknowledgement') -- category names no longer used
)
...
WHERE 
    attempt != '4th' -- Exclude rows where attempt is '4th'
ORDER BY 
...
\FloatBarrier
# The same participant received multiple Baseline Mouthwash Sample Survey SMS Reminders on the same day.
