...
GROUP BY Connect_ID, token, repeat_date
HAVING COUNT(*) > 2"
...
\FloatBarrier
## The same participant received multiple Baseline Clinical Biospecimen Survey reminders on the same day. The first attempt is category name 'Baseline Clinical Biospecimen Survey' and attempts 2-5 are under category name 'Baseline Clinical Blood and Urine Sample Survey Reminders'. Note, first contact attempt is sent at the time of the sample donation and the 2nd contact attempt is sent at 8pm that same day; for that reason we expect up to 2 contacts a day per participant. This table will identify any count higher than 2 per day.
...
LEFT JOIN `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` p
ON p.token = n.token
WHERE (category LIKE '%Baseline Clinical Biospecimen Survey%' OR category LIKE '%Baseline Clinical Blood and Urine Sample Survey Reminders%')
AND DATE(notification_time) >= (CURRENT_DATE() - 7)
#AND notificationType = 'email'
GROUP BY Connect_ID, token, repeat_date
HAVING COUNT(*) > 2"
...
\FloatBarrier
## The same participant received multiple eNewsletter notifications on the same day. 
```{r enews, echo=FALSE, message=FALSE, warning=FALSE}
enews <- "SELECT distinct(p.Connect_ID), #distinct(n.token), 
n.token,
category,
       DATE(notification_time) as repeat_date, 
       COUNT(*) as num_same_day_attempts
...
left join  `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` p
on p.token=n.token
WHERE category='eNewsletter' and date(notification_time) >= (CURRENT_DATE()-7)
GROUP BY Connect_ID, token, category, DATE(notification_time)
HAVING COUNT(*) > 1"

enews_table <- bq_project_query(project, enews)
enews_rem <- bq_table_download(enews_table, bigint = "integer64") #currently 5 - all 2


if (nrow(enews_rem) > 50) {
  cat("There are", nrow(enews_rem), "errors. This table has been made a csv and posted to Box, link in the report intro.")
  write.csv(news_rem,glue("{local_drive}Multiple_enews_rem_reminders_{currentDate}_boxfolder_{boxfolder}.csv"),row.names = F,na="")
} else {
  if(nrow(enews_rem) == 0) {
  cat("No errors\n")
} else {
  knitr::kable(enews_rem) %>% kable_styling(latex_options = "scale_down")
}
}
...
\FloatBarrier
## The same participant received multiple aniversary Newsletter notifications on the same day. 
```{r anews, echo=FALSE, message=FALSE, warning=FALSE}
anews <- "SELECT distinct(p.Connect_ID), #distinct(n.token), 
n.token,
category,
       DATE(notification_time) as repeat_date, 
       COUNT(*) as num_same_day_attempts
...
left join  `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` p
on p.token=n.token
WHERE category='anniversaryNewsletter' and date(notification_time) >= (CURRENT_DATE()-7)
GROUP BY Connect_ID, token, category, DATE(notification_time)
HAVING COUNT(*) > 1"

anews_table <- bq_project_query(project, anews)
anews_rem <- bq_table_download(anews_table, bigint = "integer64") #currently 5 - all 2


if (nrow(anews_rem) > 50) {
  cat("There are", nrow(anews_rem), "errors. This table has been made a csv and posted to Box, link in the report intro.")
  write.csv(news_rem,glue("{local_drive}Multiple_anews_rem_reminders_{currentDate}_boxfolder_{boxfolder}.csv"),row.names = F,na="")
} else {
  if(nrow(anews_rem) == 0) {
  cat("No errors\n")
} else {
  knitr::kable(anews_rem) %>% kable_styling(latex_options = "scale_down")
}
}
...
left join  `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` p
on p.token=n.token
WHERE category LIKE 'Verified, BL Survey Reminders%' and date(notification_time) >= (CURRENT_DATE()-7) and notificationType='sms'
...
left join  `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` p
on p.token=n.token
WHERE category LIKE 'Verified, BL Survey Reminders%' and date(notification_time) >= (CURRENT_DATE()-7) and notificationType='email'
and notificationType='email'
GROUP BY Connect_ID, token, category, DATE(notification_time)
HAVING COUNT(*) > 1"
...
\FloatBarrier
## The same participant received multiple (non-KPHI) Baseline Mouthwash Home Collection Kit SMS Reminders on the same day. The first attempt is category name 'Biospecimen Home Collection Kit Reminder' and attempts 2-5 are under category name 'Baseline Mouthwash Home Collection Kit Reminders'.
...
LEFT JOIN `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` p
ON p.token = n.token
WHERE (category='Baseline Mouthwash Home Collection Kit Reminders' OR category='Biospecimen Home Collection Kit Reminder' )
AND DATE(notification_time) >= (CURRENT_DATE() - 7)
...
\FloatBarrier
## The same participant received multiple (non-KPHI) Baseline Mouthwash Home Collection Kit email Reminders on the same day. The first attempt is category name 'Biospecimen Home Collection Kit Reminder' and attempts 2-5 are under category name 'Baseline Mouthwash Home Collection Kit Reminders'.
\FloatBarrier
...
\FloatBarrier
# The same participant received multiple Baseline Mouthwash Home Collection Kit SMS Reminders: KPHI  on the same day. The first attempt is category name 'Biospecimen Home Collection Kit Reminder' and attempts 2-5 are under category name 'Baseline Mouthwash Home Collection Kit Reminders: KPHI'.
...
LEFT JOIN `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` p
ON p.token = n.token
WHERE (category='Baseline Mouthwash Home Collection Kit Reminders: KPHI' OR category='Biospecimen Home Collection Kit Reminder') 
AND DATE(notification_time) >= (CURRENT_DATE() - 7)
AND notificationType = 'sms' and p.d_827220437='300267574'
GROUP BY Connect_ID, token, repeat_date
HAVING COUNT(*) > 1
...
\FloatBarrier
# The same participant received multiple Baseline Mouthwash Home Collection Kit email Reminders: KPHI  on the same day.  The first attempt is category name 'Biospecimen Home Collection Kit Reminder' and attempts 2-5 are under category name 'Baseline Mouthwash Home Collection Kit Reminders: KPHI'.
...
\FloatBarrier
# The same participant received multiple Baseline Mouthwash Sample Survey email Reminders on the same day.  The first attempt is category name 'Biospecimen Home Collection Survey Reminder' and attempts 2-5 are under category name 'Baseline Mouthwash Sample Survey Reminders'.
...
\FloatBarrier
# The same participant received multiple Baseline Mouthwash Sample Survey SMS Reminders on the same day. The first attempt is category name 'Biospecimen Home Collection Survey Reminder' and attempts 2-5 are under category name 'Baseline Mouthwash Sample Survey Reminders'.

```{r MW_Srvy_text, echo=FALSE, message=FALSE, warning=FALSE}
...
if (nrow(MW_Srvy_rem) > 50) {
  cat("There are", nrow(MW_Srvy_rem), "errors. This table has been made a csv and posted to Box, link in the report intro.")
  write.csv(MW_Srvy_rem,glue("{local_drive}Multiple_MW_Srvy_text_rem_reminders_{currentDate}_boxfolder_{boxfolder}.csv"),row.names = F,na="")
} else {
  if(nrow(MW_Srvy_rem) == 0) {
  cat("No errors\n")
} else {
...
ON
  ...
  and n.category!= 'newsletter' and n.category!='Baseline Mouthwash Sample Survey Reminders' and n.category!='Biospecimen Home Collection Kit Reminder'
and n.category!='Biospecimen Home Collection Survey Reminder'"
