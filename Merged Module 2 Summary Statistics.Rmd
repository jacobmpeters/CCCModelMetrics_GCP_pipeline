...
V1_only_vars <- colnames(M1_V1)[colnames(M1_V1) %nin% var.matched] 
V2_only_vars <- colnames(M1_V2)[colnames(M1_V2) %nin% var.matched] 
...
parts <- "SELECT Connect_ID, token, d_512820379, D_471593703, state_d_934298480, D_230663853,D_335767902, 
D_982402227, D_919254129, D_699625233, D_564964481, D_795827569, D_544150384,D_371067537, D_430551721, 
D_821247024, D_914594314,  state_d_725929722, d_126331570, d_536735468, d_663265240, d_878865966, d_684635302, 
d_167958071, d_949302066 , D_517311251, D_205553981, D_117249500, d_976570371, d_914639140, d_311580100, d_371067537 
...
(d_699625233='353358909') and (d_949302066='231311385') and d_536735468='231311385' and d_821247024='197316935'" 
...
V1_only_vars <- colnames(M2_V1)[colnames(M2_V1) %nin% var.matched] 
V2_only_vars <- colnames(M2_V2)[colnames(M2_V2) %nin% var.matched]
...
cat("Participants with Module 2 Completion:", dim(module2)[[1]])
...
bby_asp_flags <- dt_reasons_P1 %>%
  mutate(
    ...
    Skipped = if_else(rowSums(select(., starts_with("D_643807999_D_")), na.rm = TRUE) == 0 &
...
reg_asp_flags <- dt_reasons_P2 %>%
  mutate(
    ...
    Skipped = if_else(rowSums(select(., starts_with("D_275143071_")), na.rm = TRUE) == 0 &
...
ibu_flags <- dt_reasons_P3 %>%
  mutate(
    ...
    # Flag for skipped questions when all are 0 or NA
    Skipped = if_else(rowSums(select(., starts_with("D_152138929_D_")), na.rm = TRUE) == 0 &
...
acet_flags <- dt_reasons_P4 %>%
  mutate(
    ...
    # Flag for skipped questions when all are 0 or NA
    Skipped = if_else(rowSums(select(., starts_with("D_630231395_D_")), na.rm = TRUE) == 0 &
...
napx_flags <- dt_reasons_P5 %>%
  mutate(
    ...
    # Flag for skipped questions when all are 0 or NA
    Skipped = if_else(rowSums(select(., starts_with("D_655266993_D_")), na.rm = TRUE) == 0 &
...
cbx_flags <- dt_reasons_P6 %>%
  mutate(
    ...
    # Flag for skipped questions when all are 0 or NA
    Skipped = if_else(rowSums(select(., starts_with("D_509383583_D_")), na.rm = TRUE) == 0 &
...
opd_flags <- dt_reasons_P7 %>%
  mutate(
    ...
    # Flag for skipped questions when all are 0 or NA
    Skipped = if_else(rowSums(select(., starts_with("D_887435355_D_")), na.rm = TRUE) == 0 &
...
test_med_flags <- dtTESTTHER2 %>% 
  mutate(
    ...
    Skipped = if_else(rowSums(select(., starts_with("D_946849196_D_")), na.rm = TRUE) == 0 &
...
ex_flags <- data_tib_m2 %>%
  mutate(
    ...
    Skipped = if_else(rowSums(select(., starts_with("D_517976064_D_")), na.rm = TRUE) == 0 &
...
chore_flags <- data_tib_m2 %>%
  mutate(
    ...
    Skipped = if_else(rowSums(select(., starts_with("D_204421360_D_")), na.rm = TRUE) == 0 &
...
slp_cond_flags <- data_tib_m2 %>%
  mutate(
    ...
    Skipped = if_else(rowSums(select(., starts_with("D_479628567_D_")), na.rm = TRUE) == 0 &
...
slp_tmnt_flags <- dtSLPROB2 %>%
  mutate(
    ...
    # Flag for skipped questions when all are 0 or NA
    Skipped = if_else(rowSums(select(., starts_with("D_629010195_D_629010195_")), na.rm = TRUE) == 0 &
