...
funct_10_11 <- function(CID, CID_prev1, CID_prev9b, Title){
  ...
  dt_select <- data_tib_all3 %>%  filter((!!CID_SYM_P9b == 353358909) |(!!CID_SYM_P1 == 486319890) | (!!CID_SYM_P1 == 132232896)) %>%  select(Connect_ID, !!CID_SYM)
  gt_yn <- dt_select  %>% group_by(!!CID_SYM) %>% summarize(n=n(), percentage=100*n/nrow(.)) %>% ungroup() %>% mutate(answer=recode(!!CID_SYM, !!!dict)) %>%
    replace_na(list(answer="Skipped this Question")) %>%  select(answer, n, percentage)
...
#ECIGLIFEE
mhgroup9= data_tib_all3 %>%  filter(D_624111331==132232896 & !is.na(D_550244580_D_517801904) & (D_751046675==804785430 | D_242276545==353358909) & 
                                      as.numeric(data_tib_all3$D_550244580_D_517801904)<50   & !(as.numeric(data_tib_all3$D_360277949)<50) & as.numeric(data_tib_all3$AGEGRID)>39) %>% 
...
#ECIGLIFEG
mhgroup9= data_tib_all3 %>%  filter(D_624111331==132232896 & !is.na(D_550244580_D_517801904) & (D_751046675==804785430 | D_242276545==353358909) & 
                                      as.numeric(data_tib_all3$D_550244580_D_517801904)<70 & as.numeric(data_tib_all3$AGEGRID)>59) %>%  # & !(as.numeric(data_tib_all3$D_360277949)<70)) %>%  #everyone stopped before 70 
...
cigar_kind= data_tib_all3_CIGAR %>%  filter(D_815776236==353358909 | D_763164658==486319890 | D_763164658==132232896) %>% mutate(kind= case_when(D_103566006_D_103566006_D_123926260 ==1 ~ "Cigar",
