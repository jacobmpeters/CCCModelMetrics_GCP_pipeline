...
```{r tables_sepatated, warning=FALSE, echo=FALSE, message=FALSE, include=FALSE}
...
```{r tables_together, warning=FALSE, echo=FALSE, message=FALSE, results='asis'}
...
proposal_aleah <- proposal_aleah %>% 
  mutate(marketing = case_when(digital== 353358909 & traditional==353358909 ~ "Both",
                               digital== 353358909 & traditional==104430631 ~ "Digital", 
                               digital== 104430631 & traditional==353358909 ~ "Traditional",
                               digital== 104430631 & traditional==104430631 ~ "Others"))
         


demographics <- c("Age", "Race", "Sex", "Income", "Education", "Site")
market <- proposal_aleah %>% select(Connect_ID, contains("142654897"), d_914594314, all_of(demographics), marketing,traditional) 
...
#market_addition <- market_addition %>%  filter(marketing!="Others")

for (demo in demographics) {

  compare <- filter(market_addition, marketing!="Others") %>% 
    select(!!sym(demo),traditional) %>%
...
chi <- chisq.test(table(market_addition[[demo]], market_addition$traditional))
footnote <- ifelse(chi$p.value < 0.05,
                   paste0("Note: With a p-value of ", ifelse(chi$p.value < 0.001, "< 0.001", chi$p.value),
                          ", we have evidence that there is a significance in the difference of digital marketing success and traditional marketing success by ", demo, "."),
                   paste0("Note: With a p-value over 0.05, we have no evidence that there is a significance in the difference of digital marketing success and traditional marketing success by ", demo, "."))


  compare_table <- compare %>%
    modify_header() %>%
    modify_caption(paste("Marketing Strategy Counts by", demo)) %>%
    modify_footnote(all_stat_cols() ~ footnote) %>%
    as_kable_extra(escape = FALSE, addtl_fmt = TRUE) %>% kable_styling(latex_options = "scale_down")
...
```{r site_tables_separated, warning=FALSE, echo=FALSE, message=FALSE, include=FALSE}
...
site_demographics$digital <- do.call(pmax,c(site_demographics[,grepl(paste(digitals,collapse="|"),colnames(site_demographics))],na.rm=TRUE))
site_demographics$traditional <- do.call(pmax,c(site_demographics[,grepl(paste(traditionals,collapse="|"),colnames(site_demographics))],na.rm=TRUE))

site_demographics <- site_demographics %>% 
  mutate(marketing = case_when(digital== 353358909 & traditional==353358909 ~ "Both",
                               digital== 353358909 & traditional==104430631 ~ "Digital", 
                               digital== 104430631 & traditional==353358909 ~ "Traditional",
                               digital== 104430631 & traditional==104430631 ~ "Others"))
...
demographics2 <- c("Age", "Race", "Sex",  "Site")
market <- site_demographics %>% select(Connect_ID, contains("142654897"), d_914594314, all_of(demographics2), marketing,traditional) 
addtion_both <- filter(market,marketing=="Both") %>% mutate(traditional = "104430631") # to refine the traditional as digitals here
market_addition <- bind_rows(market,addtion_both)
#market_addition <- market_addition %>%  filter(marketing!="Others")

for (demo in demographics2) {
