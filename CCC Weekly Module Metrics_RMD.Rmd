...
```{r CES_BySex_ByAge_ByRace, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
...
all_sex <- knitr::kable(CES_by_Sex_df, 
             caption='Table 1.9. 2024 Connect Experience Survey Completion Rate by Site Defined Sex \n Among Verified Participants',
...
all_Age <- knitr::kable(CES_by_Age_df, 
             caption="Table 1.10. 2024 Connect Experience Survey Completion Rate by Age \n Among Verified Participants",
             row.names=FALSE, align=c("l","c","c","c","c","c"), booktabs = TRUE) %>%
    kable_styling(latex_options = "scale_down") %>%  
...
all_Race <- knitr::kable(CES_by_Race_df, 
             caption="Table 1.11. 2024 Connect Experience Survey Completion Rate by Site Defined Race \n Among Verified Participants",
             row.names=FALSE, align=c("l","c","c","c","c","c"), booktabs = TRUE) %>%
    kable_styling(latex_options = "scale_down") %>%  
...
mens_surv_status <- 
  ...
  modify_caption("Table 2.1 Menstrual Cycle Survey Submission Status for Eligible Participants") %>%
...
knitr::kable(ssn_surv_table, 
             caption='Table 3.1 Portion of SSN Given by Submission Status of SSN Survey Among Verified Participants',
             row.names=FALSE, align=c("l","c","c","c","c"), booktabs = TRUE) %>%
    kable_styling(latex_options = "scale_down") %>%  
...
BL_QOL <- all_data %>%  filter(qolcomplt=="Submitted") %>%
  mutate(after_qol = case_when(as.Date(d_843688458) <= as.Date(d_517311251) ~ "BOH Completed After QOL",
                               as.Date(d_843688458) <= as.Date(d_832139544) ~ "MRE Completed After QOL",
                               as.Date(d_843688458) <= as.Date(d_770257102) ~ "SAS Completed After QOL",
                               as.Date(d_843688458) <= as.Date(d_264644252) ~ "LAW Completed After QOL",
                               as.Date(d_843688458) <= as.Date(d_222161762) ~ "BUM Completed After QOL",
                               as.Date(d_843688458) <= as.Date(d_764863765) ~ "BU Completed After QOL",
                               as.Date(d_843688458) <= as.Date(d_195145666) ~ "MW Completed After QOL",
                               as.Date(d_843688458) <= as.Date(d_784810139) ~ "COVID Completed After QOL",
                               TRUE ~ "No Survey Submitted After QOL"),
         after_qol = factor(after_qol, levels=c("BOH Completed After QOL","MRE Completed After QOL","SAS Completed After QOL", 
                                        "LAW Completed After QOL", "BUM Completed After QOL", "BU Completed After QOL", 
                                        "MW Completed After QOL", "COVID Completed After QOL", "No Survey Submitted After QOL"))) %>% 
...
BL_QOL_table <- BL_QOL_table %>% 
  ...
  rename(`Completion Time` = after_qol, `N%` = N_percent)


knitr::kable(BL_QOL_table,
             caption = 'Table 4.11. Baseline Survey Completion On Day of or After QOL Survey Completion', 
...
BL_CES <- all_data %>%  filter(CES_status=="Submitted") %>%
  mutate(after_CES = case_when(as.Date(d_199471989) <= as.Date(d_517311251) ~ "BOH Completed After CES",
                               as.Date(d_199471989) <= as.Date(d_832139544) ~ "MRE Completed After CES",
                               as.Date(d_199471989) <= as.Date(d_770257102) ~ "SAS Completed After CES",
                               as.Date(d_199471989) <= as.Date(d_264644252) ~ "LAW Completed After CES",
                               as.Date(d_199471989) <= as.Date(d_222161762) ~ "BUM Completed After CES",
                               as.Date(d_199471989) <= as.Date(d_764863765) ~ "BU Completed After CES",
                               as.Date(d_199471989) <= as.Date(d_195145666) ~ "MW Completed After CES",
                               as.Date(d_199471989) <= as.Date(d_784810139) ~ "COVID Completed After CES",
                               TRUE ~ "No Survey Submitted After CES"),
         after_CES = factor(after_CES, levels=c("BOH Completed After CES","MRE Completed After CES","SAS Completed After CES", 
                                        "LAW Completed After CES", "BUM Completed After CES", "BU Completed After CES", 
                                        "MW Completed After CES", "COVID Completed After CES", "No Survey Submitted After CES"))) %>% 
...
BL_CES_table <- BL_CES_table %>% 
  ...
  rename(`Completion Time` = after_CES, `N%` = N_percent)


knitr::kable(BL_CES_table,
             caption = 'Table 5.8. Baseline Survey Completion On Day of or After CES Survey Completion', 
             booktabs=T, linesep = "", row.names=FALSE, align=c("l", "c")) %>% 
...
knitr::kable(BL_lang_E, format.args = list(big.mark = ","), digits=2, 
             caption='6.4.a Time (in minutes) from Start to Submit of Initial Survey Modules for English',
