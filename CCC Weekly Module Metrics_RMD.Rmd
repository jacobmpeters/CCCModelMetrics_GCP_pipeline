...
d_784810139, d_176068627, d_389890053, d_609630315
...
\newpage
\FloatBarrier
# Connect Screening History Survey
...
CSH_comp <- CSH_comp %>%
  ...
  modify_caption("Table 5.1. Completion of Connect Screening History Survey Among Eligible Participants") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
knitr::kable(CSH_com_df, 
             caption='Table 5.2. Completion of Connect Screening History Survey by Baseline Data Completeness',
             row.names=FALSE, align=c("l","c","c","c","c"), booktabs = TRUE) %>%
  kable_styling(latex_options = "scale_down") %>%  
...
knitr::kable(CSH_TM, 
             caption = 'Table 5.3. Connect Screening History Survey Completion Time from Start to Submit \n Among Eligible Participants', 
             booktabs=T, linesep = "", row.names=FALSE, align=c("l", "c")) %>% 
...
trig_csh <- bq_project_query(project,
                             query="SELECT distinct(Connect_ID), notification_time FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.notifications_JP` notif
...
trigger <- bigrquery::bq_table_download(trig_csh,bigint = "integer64")


all_data$Connect_ID <- as.numeric(all_data$Connect_ID)
trigger$Connect_ID <- as.numeric(trigger$Connect_ID)

trigger_time <- left_join(all_data, trigger, by="Connect_ID")
...
knitr::kable(CSH_notif_table, 
             caption = 'Table 5.4. Connect Screening History Survey Completion from Trigger to Submit for \n Verified Participants, in Days', 
             row.names=FALSE, format.args = list(big.mark = ","), linesep = "", 
             booktabs = T, align=c("l", "c", "c", "c", "c", "c", "c", "c", "c")) %>% 
  kable_styling(latex_options = c("scale_down","hold_position")) 
...
\newpage
\FloatBarrier
```{r BL_After_CSH, echo=FALSE, warning=FALSE, message=FALSE}
...
queries <- list(
  ...
  CSH_lang_c = "SELECT pts.Connect_ID, csh.d_784119588, d_255077064
                FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.experience2024_JP` csh
                left join `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` pts
