...
CES_by_Sex <- all_data %>% 
  select(sex, BL_comp) %>% dplyr::arrange(sex,BL_comp) %>% 
  tbl_cross(
  row = sex,
  col = BL_comp,
  label = list(BL_comp ~ "Baseline Survey \n Completion",sex~"Sex"),
  percent = "row",
  digits=c(0,2),
  margin_text="Total Verified Participants") 

#Need only the total column to be a column percentage
CES_by_Sex[["table_body"]]$stat_0 <- sapply(strsplit(CES_by_Sex[["table_body"]]$stat_0," "),"[",1)

CES_by_Sex_df <- as.data.frame(CES_by_Sex)[2:5,]
colnames(CES_by_Sex_df)[1] <- "Sex"

CES_by_Sex_df$`Total Verified Participants` <- as.numeric(gsub(",", "", CES_by_Sex_df$`Total Verified Participants`))

total_participants <- CES_by_Sex_df$`Total Verified Participants`
percentages <- round((total_participants / total_participants[length(total_participants)]) * 100, digits = 2)


CES_by_Sex_df$`Total Verified Participants` <- paste0(format(total_participants, big.mark = ","), " (", percentages, "%)")

colnames(CES_by_Sex_df)[6] <- "Total Verified \n Participants"
...
CES_by_Age <- all_data %>% 
  select(age, BL_comp) %>% dplyr::arrange(age,BL_comp) %>% 
  tbl_cross(
  row = age,
  col = BL_comp,
  label = list(BL_comp ~ "Baseline Survey \n Completion",age~"Age"),
...
#Need only the total column to be a column percentage
CES_by_Age[["table_body"]]$stat_0 <- sapply(strsplit(CES_by_Age[["table_body"]]$stat_0," "),"[",1)

CES_by_Age_df <- as.data.frame(CES_by_Age)[2:11,]
colnames(CES_by_Age_df)[1] <- "Age"

CES_by_Age_df$`Total Verified Participants` <- as.numeric(gsub(",", "", CES_by_Age_df$`Total Verified Participants`))

total_participants <- CES_by_Age_df$`Total Verified Participants`
percentages <- round((total_participants / total_participants[length(total_participants)]) * 100, digits = 2)


CES_by_Age_df$`Total Verified Participants` <- paste0(format(total_participants, big.mark = ","), " (", percentages, "%)")

colnames(CES_by_Age_df)[6] <- "Total Verified \n Participants"
...
CES_by_Race <- all_data %>% 
  select(race, BL_comp) %>% dplyr::arrange(race,BL_comp) %>% 
  tbl_cross(
  row = race,
  col = BL_comp,
...
#Need only the total column to be a column percentage
CES_by_Race[["table_body"]]$stat_0 <- sapply(strsplit(CES_by_Race[["table_body"]]$stat_0," "),"[",1)

CES_by_Race_df <- as.data.frame(CES_by_Race)[2:5,]
colnames(CES_by_Race_df)[1] <- "Race"

CES_by_Race_df$`Total Verified Participants` <- as.numeric(gsub(",", "", CES_by_Race_df$`Total Verified Participants`))

total_participants <- CES_by_Race_df$`Total Verified Participants`
percentages <- round((total_participants / total_participants[length(total_participants)]) * 100, digits = 2)


CES_by_Race_df$`Total Verified Participants` <- paste0(format(total_participants, big.mark = ","), " (", percentages, "%)")

colnames(CES_by_Race_df)[6] <- "Total Verified \n Participants"
...
```{r Knit_CES_BySex_ByAge_ByRace, echo=FALSE, warning=FALSE, message=FALSE, results='eval'}

# add_footnote(all_sex, "Note: Sex defined as the de-identified sex data reported by sites at time of invitation.", 
#              notation="none")
# 
# add_footnote(all_Age, "Note: Age defined as the de-identified age data reported by sites at time of invitation, not current age at time of survey completion.", 
#              notation="none")
# 
# add_footnote(all_Race, "Note: Race defined as the de-identified race data reported by sites at time of invitation.", 
#              notation="none")
```
...
ssn_surv_table <- ssn_surv %>% 
  tbl_cross(
    ...
    missing="ifany",
    margin_text="Total Verified")  

ssn_surv_table[["table_body"]]$stat_0 <- sapply(strsplit(ssn_surv_table[["table_body"]]$stat_0," "),"[",1)

ssn_surv_table <- as.data.frame(ssn_surv_table)[2:5,]
colnames(ssn_surv_table)[1] <- "Submission Status"

ssn_surv_table$`Total Verified` <- as.numeric(gsub(",", "", ssn_surv_table$`Total Verified`))

total_participants <- ssn_surv_table$`Total Verified`
percentages <- round((total_participants / total_participants[4]) * 100, digits = 2)


ssn_surv_table$`Total Verified` <- paste0(format(total_participants, big.mark = ","), " (", percentages, "%)")
...
#   modify_caption("Table 3.1 Portion of SSN Given by Submission Status of SSN Survey \n Among Verified Participants") %>%
...
QOL_comp_BL <- as.data.frame(QOL_comp_BL)[2:6,]
colnames(QOL_comp_BL)[1] <- "Survey Completion"

QOL_comp_BL$`Total QOL Eligible Participants` <- as.numeric(gsub(",", "", QOL_comp_BL$`Total QOL Eligible Participants`))

total_participants <- QOL_comp_BL$`Total QOL Eligible Participants`
percentages <- round((total_participants / total_participants[5]) * 100, digits = 2)


QOL_comp_BL$`Total QOL Eligible Participants` <- paste0(format(total_participants, big.mark = ","), " (", percentages, "%)")
...
#Need only the total column to be a column percentage
QOL_comp_BL_after[["table_body"]]$stat_0 <- sapply(strsplit(QOL_comp_BL_after[["table_body"]]$stat_0," "),"[",1)

QOL_comp_BL_after <- as.data.frame(QOL_comp_BL_after)[2:6,]
colnames(QOL_comp_BL_after)[1] <- "Survey Completion"

QOL_comp_BL_after$`Total QOL Eligible Participants` <- as.numeric(gsub(",", "", QOL_comp_BL_after$`Total QOL Eligible Participants`))

total_participants <- QOL_comp_BL_after$`Total QOL Eligible Participants`
percentages <- round((total_participants / total_participants[5]) * 100, digits = 2)


QOL_comp_BL_after$`Total QOL Eligible Participants` <- paste0(format(total_participants, big.mark = ","), " (", percentages, "%)")
...
#Need only the total column to be a column percentage
QOL_comp_BL_before[["table_body"]]$stat_0 <- sapply(strsplit(QOL_comp_BL_before[["table_body"]]$stat_0," "),"[",1)

QOL_comp_BL_before <- as.data.frame(QOL_comp_BL_before)[2:6,]
colnames(QOL_comp_BL_before)[1] <- "Survey Completion"

QOL_comp_BL_before$`Total QOL Eligible Participants` <- as.numeric(gsub(",", "", QOL_comp_BL_before$`Total QOL Eligible Participants`))

total_participants <- QOL_comp_BL_before$`Total QOL Eligible Participants`
percentages <- round((total_participants / total_participants[5]) * 100, digits = 2)


QOL_comp_BL_before$`Total QOL Eligible Participants` <- paste0(format(total_participants, big.mark = ","), " (", percentages, "%)")
...
qol_ver<- all_data %>%  filter(qolcomplt != "Not Eligible") %>% 
  mutate(ver_surv = case_when(as.Date(d_914594314) < "2023-01-01" ~ "More than 1 Year",
    ...
    tbl_cross(
      row = ver_surv,
      col = qolcomplt,
      label = list(qolcomplt ~ "Survey Completion",ver_surv~"Time from Verification to QOL Survey Release"),
      percent = "row",
      digits=c(0,2),
      margin_text="Total QOL Eligible Participants")
...
#Need only the total column to be a column percentage
qol_ver[["table_body"]]$stat_0 <- sapply(strsplit(qol_ver[["table_body"]]$stat_0," "),"[",1)

qol_ver_df <- as.data.frame(qol_ver)[2:6,]
colnames(qol_ver_df)[1] <- "Time from Verification to QOL Survey Release"

qol_ver_df$`Total QOL Eligible Participants` <- as.numeric(gsub(",", "", qol_ver_df$`Total QOL Eligible Participants`))

total_participants <- qol_ver_df$`Total QOL Eligible Participants`
percentages <- round((total_participants / total_participants[length(total_participants)]) * 100, digits = 2)


qol_ver_df$`Total QOL Eligible Participants` <- paste0(format(total_participants, big.mark = ","), " (", percentages, "%)")
...
all_data <- all_data %>% mutate(CES_status = case_when(d_956490759==972455046	~ "Not Started",
...
CES_comp_BL <- all_data %>%  filter(CES_status!="Not Eligible") %>%  
  mutate(CES_status = factor(CES_status, levels=c("Not Started", "Started", "Submitted")),
         completion = factor(completion, levels=c("Completed both survey and sample(s)","Completed all baseline survey modules 1-4, no baseline samples", 
                                                  "Completed any baseline sample, did not complete baseline survey","Completed neither survey or sample(s)"))) %>% 
  select(completion, CES_status) %>% dplyr::arrange(completion,CES_status) %>% tbl_cross(
  row = completion,
  col = CES_status,
  label = list(CES_status ~ "Survey Completion",completion~"Completion"),
  percent = "row",
  digits=c(0,2),
  margin_text="Total CES Eligible Participants") 
...
#Need only the total column to be a column percentage
CES_comp_BL[["table_body"]]$stat_0 <- sapply(strsplit(CES_comp_BL[["table_body"]]$stat_0," "),"[",1)

CES_com_df <- as.data.frame(CES_comp_BL)[2:6,]
colnames(CES_com_df)[1] <- "Completion"

CES_com_df$`Total CES Eligible Participants` <- as.numeric(gsub(",", "", CES_com_df$`Total CES Eligible Participants`))

total_participants <- CES_com_df$`Total CES Eligible Participants`
percentages <- round((total_participants / total_participants[5]) * 100, digits = 2)


CES_com_df$`Total CES Eligible Participants` <- paste0(format(total_participants, big.mark = ","), " (", percentages, "%)")
