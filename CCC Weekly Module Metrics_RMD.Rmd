...
all_data <- all_data %>%  mutate(Site = case_when(d_827220437==472940358 ~ "Baylor Scott & White Health",
...
all_data$Site <- factor(all_data$Site, levels=c("Baylor Scott & White Health","HealthPartners", "Henry Ford","Marshfield","Sanford","UChicago","KP Colorado","KP Georgia", "KP Hawaii","KP Northwest"))
...
knitr::kable(comp_by_site_totals , caption='Table 1.Completion of Initial Surveys by Site', row.names=FALSE,align=c("l","c","c","c"),
             #col.names=c("", "All Baseline Modules Completed", "One or More Baseline \n Modules Not Completed", "Total"), escape=F,
             booktabs = TRUE) %>%  add_indent(seq(1, total_rows - 1))  %>% kable_styling(latex_options = "scale_down") # %>%  column_spec(3:4, width = "4cm")
#
...
total_rows3 <- nrow(all_srvy_comp_totals)
knitr::kable(table6.1_sum, caption='Table 6.1 BOH Section Initial Survey Submission Time \n from Start to Submit for Verified Participants', col.names=c("Submission Time", "Total Participants", "Cumulative Percentage"), row.names=FALSE, align=c("l","c","c"),digits=1, booktabs = TRUE) %>%  add_indent(seq(1, total_rows3 - 1))  %>% kable_styling(latex_options = "hold_position")
...
total_rows4 <- nrow(all_srvy_comp_totals)
knitr::kable(table6.2_sum, caption='Table 6.2 MRE Section Initial Survey Submission Time \n from Start to Submit for Verified Participants', col.names=c("Submission Time", "Total Participants", "Cumulative Percentage"), row.names=FALSE, align=c("l","c"),digits=1, booktabs = TRUE) %>%  add_indent(seq(1, total_rows4 - 1))  %>% kable_styling(latex_options = "hold_position")
...
total_rows5 <- nrow(all_srvy_comp_totals)
knitr::kable(table6.3_sum, caption='Table 6.3 SAS Section Initial Survey Submission Time \n from Start to Submit for Verified Participants', col.names=c("Submission Time", "Total Participants", "Cumulative Percentage"), row.names=FALSE, align=c("l","c"),digits=1, booktabs = TRUE) %>%  add_indent(seq(1, total_rows5 - 1)) %>% kable_styling(latex_options = "hold_position")
...
total_rows7 <- nrow(all_srvy_comp_totals)
knitr::kable(table6.4_sum, caption='Table 6.4 LAW Section Initial Survey Submission Time \n from Start to Submit for Verified Participants', col.names=c("Submission Time", "Total Participants", "Cumulative Percentage"), row.names=FALSE, align=c("l","c"),digits=1, booktabs = TRUE) %>%  add_indent(seq(1, total_rows7 - 1)) %>% kable_styling(latex_options = "hold_position")
