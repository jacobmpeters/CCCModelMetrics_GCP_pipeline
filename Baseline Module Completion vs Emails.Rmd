---
title: "Baseline Survey Completion Responses"
...
# #Rounding contact date up to 4th, 7th, 16th, 25th, 40th
# comp_time <- reminders %>%
#   filter(reminders$d_100767870 == '353358909' & !is.na(d_914594314) & !is.na(latest_module_date)) %>%
#   mutate(verification_date = as.POSIXct(d_914594314), #, origin = "1970-01-01"
...
#     days_difference <= 0 ~ "Survey Completed before Contact 1",
#     between(days_difference, 1, 3) ~ "Survey Completed after Contact 1, before Contact 2",
...
# 
#Not rounding up, assuming they received the reminders on those dates regardless of time of day
comp_time <- reminders %>%
  filter(reminders$d_100767870 == '353358909' & !is.na(d_914594314) & !is.na(latest_module_date)) %>%
  mutate(verification_date = as.POSIXct(d_914594314), #, origin = "1970-01-01"
         module_completion_date = as.POSIXct(latest_module_date), #, origin = "1970-01-01"
...
#Using hours of the day instead of rounding by day, and assuming reminders are sent on the 0th, 4th, 7th, 16th, 25th, 40th
# comp_time <- reminders %>%
#   filter(reminders$d_100767870 == '353358909' & !is.na(d_914594314) & !is.na(latest_module_date)) %>%
#   mutate(verification_date = as.POSIXct(d_914594314), #, origin = "1970-01-01"
#          module_completion_date = as.POSIXct(latest_module_date), #, origin = "1970-01-01"
#          days_difference = round(as.numeric(difftime(module_completion_date, verification_date, units = "hours")))) %>%
#   mutate(time_frame = case_when(
#     days_difference <= 0 ~ "Survey Completed before Contact 1",
#     between(days_difference, 24, 72) ~ "Survey Completed after Contact 1, before Contact 2",
#     between(days_difference, 96, 144) ~ "Survey Completed after Contact 2, before Contact 3",
#     between(days_difference, 168, 360) ~ "Survey Completed after Contact 3, before Contact 4",
#     between(days_difference, 384, 576) ~ "Survey Completed after Contact 4, before Contact 5",
#     between(days_difference, 600, 960) ~ "Survey Completed after Contact 5, before Contact 6",
#     days_difference >= 984 ~ "Survey Completed after Contact 6"
#   )) %>%
#   group_by(time_frame) %>%
#   summarise(n = n_distinct(Connect_ID))


comp_time$time_frame <- factor(comp_time$time_frame, levels=c("Survey Completed before Contact 1","Survey Completed after Contact 1, before Contact 2","Survey Completed after Contact 2, before Contact 3",
...
reminders_ct <- reminders %>% mutate(cnt_number=case_when(attempt=="1st contact" ~ "Sent reminder email Contact 1",
 ...
 attempt_counts <- reminders_ct %>%
     ...
     slice_head() %>%  # Keep only the first row for each unique Connect_ID and contacts_number- that way the case_when doesn't eliminat participants with each case
     group_by(cnt_number) %>%
...
EligibleNumb <- c(sum(all_participants$n)- (sum(comp_time[1,2]) + sum(comp_time[2,2])), #eligible2= all participants-those who completed before contact 2
                  ...
                  Sent_rows[5,2] - sum(comp_time[6,2])) #eligible for 6 =sent contact5-completed before contact 6
