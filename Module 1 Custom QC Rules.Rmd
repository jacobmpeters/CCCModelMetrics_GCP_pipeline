---
title: "Module 1 Custom QC Rules"
...
length(recr_m1$Connect_ID[which(recr_m1$Connect_ID %in% m1_v1_only$Connect_ID )]) #1364 03282023 # 1370 05022023
length(recr_m1$Connect_ID[which(recr_m1$Connect_ID %in% m1_v2_only$Connect_ID )]) #4870 03282023 # 5731 05022023
...
m1_comb <- m1_combined_v1v2 %>% arrange(desc(version)) 


m1_comb_nodup <- m1_comb[!duplicated(m1_comb$Connect_ID),] 
table(m1_comb_nodup$version)



parts <- "SELECT Connect_ID, token, D_512820379, D_471593703, state_d_934298480, D_230663853, d_949302066, d_536735468 ,d_976570371 , d_663265240, d_827220437
...
module1= left_join(m1_comb_nodup, parts_data, by="Connect_ID") 
dim(module1)
...
\FloatBarrier
### If Module 1 isn't completed, no other module can be started or completed
...
\FloatBarrier
### If 'none of the above' is selected for cardiovascular conditions, no other cardiovascular condition can be chosen. Note this table is very wide, so if there are no errors found all that will be diplayed is "No errors".
...
\FloatBarrier
### If 'none of the above' is selected for digestive conditions, no other digestive condition can be chosen. Note this table is very wide, so if there are no errors found all that will be diplayed is "No errors".
...
\FloatBarrier
### If 'none of the above' is selected for STDs and Other Infectious conditions, no other STD or condition can be chosen. Note this table is very wide, so if there are no errors found all that will be diplayed is "No errors".
...
\FloatBarrier
### If 'none of the above' is selected for urinary conditions, no other urinary condition can be chosen. Note this table is very wide, so if there are no errors found all that will be diplayed is "No errors".
...
\FloatBarrier
### If 'none of the above' is selected for surgerical prodedures, no other surgeries can be chosen. Note this table is very wide, so if there are no errors found all that will be diplayed is "No errors".
...
\FloatBarrier
### If 'none of the above' is selected for breast surgeries, no other breast surgeries can be chosen. Note this table is very wide, so if there are no errors found all that will be diplayed is "No errors".
...
\FloatBarrier
### Alternative contact mobile number (SrvBOH_AltContCell_v1r0) must be numeric and 10 digits
...
\FloatBarrier
### Alternative contact home number must be numeric and 10 digits
...
\FloatBarrier
### ZIP code should be no more than 9 digits; 5 with primary ZIP code and 9 if full ZIP code given 
```{r alt_home_phone, echo=FALSE, warning=FALSE, message=FALSE}
