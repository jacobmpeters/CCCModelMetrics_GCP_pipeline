---
title: "UC Module 4 Missingness Analysis"
...
where Connect_ID IS NOT NULL and d_827220437='809703864'"
...
cat("Module 4 Completed by : ", dim(module4)[[1]], " UC Participants")
```
...
dt_home_add3_1 <- dt_home_add3_1[2,]
...
dt_home_add3_3 <- dt_home_add3_3[2,]
...
dt_home_add3_4 <- dt_home_add3_4[2,]
...
dt_home5_water_summary <- dt_home5_water_summary[c(2,3),]
...
dt_childhome_summary[3,1] <- "Skipped Street Name" 
dt_childhome_summary[3,2] <- sum(is.na(childhome$D_632533534_D_802585033))
dt_childhome_summary[3,3] <- (sum(is.na(childhome$D_632533534_D_802585033))/dim(childhome)[[1]])*100

dt_childhome_summary[4,1] <- "Skipped City"
dt_childhome_summary[4,2] <- sum(is.na(childhome$D_632533534_D_128827522))
dt_childhome_summary[4,3] <- (sum(is.na(childhome$D_632533534_D_128827522))/dim(childhome)[[1]])*100

dt_childhome_summary[5,1] <- "Skipped State"
dt_childhome_summary[5,2] <- sum(is.na(childhome$D_632533534_D_439447560))
dt_childhome_summary[5,3] <- (sum(is.na(childhome$D_632533534_D_439447560))/dim(childhome)[[1]])*100

dt_childhome_summary[6,1] <- "Skipped ZIP"
dt_childhome_summary[6,2] <- sum(is.na(childhome$D_632533534_D_286781627))
dt_childhome_summary[6,3] <- (sum(is.na(childhome$D_632533534_D_286781627))/dim(childhome)[[1]])*100

dt_childhome_summary[7,1] <- "Skipped Country"
dt_childhome_summary[7,2] <- sum(is.na(childhome$D_632533534_D_733929451))
dt_childhome_summary[7,3] <- (sum(is.na(childhome$D_632533534_D_733929451))/dim(childhome)[[1]])*100
...
CURWORKT2A_H= Mod4 %>%   
  filter( #(is.na(D_440093675_D_593010802) & D_635467479_D_635467479_D_158795242==1 | 
             ...
             # is.na(D_440093675_D_662584384) & D_635467479_D_635467479_D_902077989==1 ) &
...
dt_CURWORKT2A_H_summary <- dt_CURWORKT2A_H_summary[c(2,3,4,5,6,7,8),]
...
PREWORKRES= Mod4 %>%  filter(Mod4$D_995620555==353358909 & is.na(D_980007418) ) %>%  mutate(Address1= case_when(is.na(D_980007418) ~"Skipped PREWORKRES",
                                                                                          TRUE ~ "Answered"))
...
CURSCHT6= STU %>%  filter(STU$D_449752653==353358909 & is.na(STU$D_940631293)) %>% 
...
CURSCHT7= STU %>%  filter(STU$D_449752653==353358909 & is.na(STU$D_496801729)) %>%  
