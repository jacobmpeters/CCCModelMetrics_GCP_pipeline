...
gt_yn <- gt_yn %>%  dplyr::mutate(np= paste0(n, "(", round(percentage, digits=1), "%)"))

gt_yn$answer <- factor(gt_yn$answer) #line causing the issues

summary_row_simple1 <- gt_yn %>%
  summarise(np = paste0(sum(n, na.rm = TRUE), " (100%)"),
            answer = "Total") %>% select(answer, np)

gt_yn <- gt_yn %>% select(answer, np)

# Bind summary row to dt_simple_funct1_summary
dt_simple_funct1_summary <- bind_rows(gt_yn, summary_row_simple1)

#Get last row number for bolding the total
last_row_index <- nrow(dt_simple_funct1_summary)

  dt_simple_funct1_summary %>%  gt::gt(rowname_col = "row_lab")  %>%
  #fmt_number(columns = "percentage", decimals = 2) %>%
  tab_header(title = md(Title)) %>%
  cols_label(answer = md("**Answer**"), np = md("**N (%)**")) %>%
  # summary_rows(
  ...
  tab_options(
   ...
   tab_style(
      ...
      locations = cells_body(
        rows = seq_len(nrow(dt_simple_funct1_summary)) != nrow(dt_simple_funct1_summary),
        columns = vars(answer)
      )
    )
...
#gt_yn$answer <- factor(gt_yn$answer,levels=List_of_Levels) #line causing the issues

summary_row_one_logic1 <- gt_yn %>%
    summarise(np = paste0(sum(n, na.rm = TRUE), " (100%)"),
              answer = "Total") %>% select(answer, np)

gt_yn <- gt_yn %>% select(answer, np)

# Bind summary row to dt_one_logic_funct1_summary
dt_one_logic_funct1_summary <- bind_rows(gt_yn, summary_row_one_logic1)

#Get last row number for bolding the total
last_row_index <- nrow(dt_one_logic_funct1_summary) 

  
 dt_one_logic_funct1_summary %>%  gt::gt(rowname_col = "row_lab")  %>%  
  #fmt_number(columns = "percentage", decimals = 2) %>% 
  tab_header(title = md(Title)) %>% 
   cols_label(answer = md("**Answer**"), np = md("**N (%)**")) %>%
 ...
 tab_style(
    ...
    locations = cells_body(
        rows = seq_len(nrow(dt_one_logic_funct1_summary)) != nrow(dt_one_logic_funct1_summary), 
        columns = vars(answer)
    ) 
) 
}
...
data_tib_base$D_407056417 <- factor(data_tib_base$D_407056417,levels=c("Female", "Male", "Intersex or Other", "Skipped this Question"))
...
dt_appearence_summ <- style  %>% dplyr::group_by(descr) %>%  dplyr::summarize(n=n(), percentage=100*n/nrow(.))   %>% dplyr::ungroup() %>%  dplyr::select(descr, n, percentage)
...
simple_funct1(D_555481393_D_555481393, "SEXORIET: Sexual Orienation")
...
```{r MomCancerFinalT, warning=FALSE, echo=FALSE, message=FALSE}
...
dt___dad_cnc_summary <- dt_dad_cnc_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
...
dtLIQ1_summary %>%  gt::gt(rowname_col = "row_lab") %>%
  fmt_number(columns = "percentage", decimals = 2) %>%
...
dtWINE1_summary %>%  gt::gt(rowname_col = "row_lab") %>%
  fmt_number(columns = "percentage", decimals = 2) %>%
...
dtOALC1_summary %>%  gt::gt(rowname_col = "row_lab") %>%
  fmt_number(columns = "percentage", decimals = 2) %>%
