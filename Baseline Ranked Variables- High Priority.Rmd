...
d_100767870 FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` where Connect_ID IS NOT NULL and (d_512820379='486306141' OR d_512820379='854703046') and (D_919254129='353358909') and (D_699625233='353358909') and (d_949302066='231311385')" 
...
dict <- list("104430631"="No", "353358909"="Yes", "589959753"="A little shorter", "623310018"="A lot shorter", "978204320"= "Grade School(Grades 1-8)", "935502060"="Some High School, No Diploma", "404564707"= "High School Graduate or GED", "432193665"="Some College, No Degree", "890756124"="Technical or Trade School After High School", "766964355"="Associate's Degree", "875342283"="Bachelor's Degree", "598242454"="Advanced Degree", "709650750"="Biological Child", "810290359"="Adopted Child", "509607548"="Step Child","784922042"="Related to me in some other way", "288105839"=" Yes, fraternal twins", "626558982"="Yes, triplets or higher multiple birth", "178420302"="Do not Know", "536341288"="Female", "576796184"="Intersex or Other", "654207589"="Male","555374628"="Full Sibling", "871673221"="Half Sibling, Same Mother",  "198654048"="Half Sibling, Same Father", "807835037"="Other", "274062548"="More than 3 months ago", "958538953"="In the past 3 months", "317567178"="In the past month", "484055234"="In the past year",  "802197176"="More than 1 year ago", "[]"="Answered with number", "[178420302]"="Answered Don't Know", "274062548"="More than 3 months ago","958538953"="In the past 3 months",  "490796638"="Not Flexible","719954633"="Extremely Flexible","929148006"="A Little Flexible","988873962"="Somewhat Flexible", "764708931"="Very flexible","0"="No", "1"="Yes", "536341288"="Female", "218837028"="Woman (New Response)", "983318667"="Man (New Response)"  , "654207589"="Male","746038746"="Declined", "805712793"="Genderqueer","486192236"="Non-Binary", "807835037"="Other","873138103"="Transgender Woman","405267600"="Transgender Man", "220083334"="Very Feminine","541300533"="Mostly Feminine","878286618"="Somewhat Feminine","910745276"="Equally feminine and masculine","915528806"="Very masculine","265452386"="Mostly Masculine","510148951"="Somewhat Masculine","271882746"="Straight or heterosexual", "903084185"="Lesbian, gay, or homosexual", "999994434"="Bisexual",
  "727200870"="Asexual", "854349138"="Pansexual", "832978839"="Mostly Straight", "197935377"="Queer", "410008111"="Two-Spirit", "831942158"="Questioning", "210894509"="No labels",  "209571450" = "35,000-49,000","212249150" = "50,000-74,000","374508062" = "0-9,000",
  "530094502" = "Error- Unlisted Response Selected", "742032816" = "200,000+", "745561936" = "25,000-34,000","746038746" = "Prefer not to Answer","777814771" = "75,000-99,000","913602274" = "150,000-199,000", "922395188" = "100,000-149,000","976555124" = "10,000-24,000", "670680466"= "Excellent","565881164"= "Very good","719933364"= "Good","131550264"= "Fair","138752522"= "Poor") 

simple_funct1 <- function(CID, Title){  
  CID_SYM <- rlang::ensym(CID)
  dt_select <- data_tib_base %>%  select(Connect_ID, !!CID_SYM)
  gt_yn <- data_tib_base  %>% dplyr::group_by(!!CID_SYM) %>% dplyr::summarize(n=n(), percentage=100*n/nrow(.)) %>% dplyr::ungroup() %>% dplyr::mutate(answer=recode(!!CID_SYM, !!!dict)) %>% replace_na(list(answer="Skipped this Question")) %>%  dplyr::select(answer, n, percentage)


  gt_yn %>%  gt::gt()  %>%
  fmt_number(columns = "percentage", decimals = 2) %>%
  tab_header(title = md(Title)) %>%
  cols_label(n= md("**Number of Participants**"), answer = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>%
...
#levels_list <- c("Female", "Male", "Intersex or Other", "Skipped this Question")
...
#"Female", "Male", "Intersex or Other", "Skipped this Question"
...
#gt_total_func(dt_sex2_summary, "body_part", "Body Part of Intersex Participants")
...
dt_language_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(n= md("**Number of Participants**"), language = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>% 
  # summary_rows( groups=F,
  #   column = c(n,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
skincnc_type= merged %>%  filter(merged$D_537153788==353358909) %>%  mutate(Type= case_when(D_508846529_D_864052438==1 & D_508846529_D_323177352==0~"Basal Cell",
...
dt_mgr1 <- mgr1 %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(Frequency= md("**Number of Participants**"), Percentage=md("**Percentage of <br> Participants**"),  Condition = md("**Answer**")) %>% 
  # summary_rows( groups=F,
...
dt_mgr2 <- mgr2 %>%  gt::gt(rowname_col = "row_lab") %>% 
  ...
  cols_label(Frequency= md("**Number of Participants**"), Percentage=md("**Percentage of <br> Participants**"),  lung_dx = md("**Answer**")) %>% 
  # summary_rows(
...
```
```{r MhGroup3Table, warning=FALSE, echo=FALSE, message=FALSE}
...
dt_mgr3 <- mgr3 %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(Frequency= md("**Number of Participants**"), Percentage=md("**Percentage of <br> Participants**"), dgst_dx = md("**Answer**")) %>% 
  # summary_rows(
  #   column = c(participants,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
```
```{r MhGroup4Table, warning=FALSE, echo=FALSE, message=FALSE}
...
dt_mgr4 <- mgr4 %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(Frequency= md("**Number of Participants**"),Percentage=md("**Percentage of <br> Participants**"), thyrd_dx = md("**Answer**")) %>% 
  # summary_rows(
  #   column = c(participants,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
## MH GROUP5
```{r MhGroup5, warning=FALSE, echo=FALSE, message=FALSE, include=FALSE}
...
dt_mgr5 <- mgr5 %>%  gt::gt(rowname_col = "row_lab") %>%
  ...
  cols_label(Frequency= md("**Number of Participants**"), Percentage=md("**Percentage of <br> Participants**"), kidn_dx = md("**Answer**")) %>% 
  # summary_rows(
  #   column = c(participants,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
## MH GROUP6
```{r MhGroup6, warning=FALSE, echo=FALSE, message=FALSE, include=FALSE}
...
dt_mgr6 <- mgr6 %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(Frequency= md("**Number of Participants**"), Percentage=md("**Percentage of <br> Participants**"), systm_dx = md("**Answer**")) %>% 
  # summary_rows(
  #   column = c(participants,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
## MH GROUP7
```{r MhGroup7, warning=FALSE, echo=FALSE, message=FALSE, include=FALSE}
...
dt_mgr7 <- mgr7 %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(Frequency= md("**Number of Participants**"), Percentage=md("**Percentage of <br> Participants**"),  sti_dx = md("**Answer**")) %>% 
  # summary_rows(
  #   column = c(participants,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
dt_mgr8 <- mgr8 %>%  gt::gt(rowname_col = "row_lab") %>% 
  ...
  cols_label(Frequency= md("**Number of Participants**"), Percentage=md("**Percentage of <br> Participants**"),  urinrp_dx = md("**Answer**")) %>% 
  # summary_rows(
  #   column = c(participants,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
dt_mgr9 <- mgr9 %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(Frequency= md("**Number of Participants**"),  Percentage=md("**Percentage of <br> Participants**"), srgy_dx = md("**Answer**")) %>% 
  # summary_rows(
  #   column = c(participants,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
```{r MomCancerTable, include=FALSE}
...
```{r MomCancerFinalT, warning=FALSE, echo=FALSE, message=FALSE, include=FALSE}
...
dt_dad_cnc_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(n= md("**Number of Participants**"), cancer = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>% 
  grand_summary_rows(columns=c(n, percentage),fns = ~sum(.,na.rm = T))|>
  tab_options(
      stub.font.weight = "bold"
    ) %>%
...
```{r DadCancSetUp, include=FALSE}

# tmpDADCANC <- data_tib_base %>%  filter(data_tib_base$D_659784914==353358909)
# tmpMC2<- tmpDADCANC[, grepl("173240848", colnames(tmpDADCANC))]
# tmpM2 <- tmpMC2 %>%  select(-"D_173240848_D_195093589", -"D_173240848_D_173240848_D_210004486") #-"D_173240848_D_173240848_D_195093589",  in v2 but has no data
# 
# DADcnc_dx <- NULL
# for (i in 1:22){
#   dt <- as.data.frame(tab1(tmpM2[,i])$output.table)
...
```{r DadCancTable, warning=FALSE, echo=FALSE, message=FALSE, include=FALSE}
...
dt_income_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(n= md("**Number of Participants**"), status = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>% 
...
hist_funct <- function(CID,Title, Xlab, Ylab) {
  CID_SYM <- rlang::ensym(CID)
  merge_m2 %>% mutate(!!CID_SYM:=round(as.numeric(as.character(!!CID_SYM)), digits=0)) %>% 
     filter(!is.na(!!CID_SYM)) %>%
...
simple_funct2 <- function(CID, Title, List_of_Levels){  
  CID_SYM <- rlang::ensym(CID)
...
#gt_total_func(dt_agep_summary, "age", "HORNONCISWOM: Used Hormones to Make Body Look More Feminine")
...
dt_alcl_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
  tab_header(title = md(Title)) %>% 
  cols_label(n= md("**Number of Participants**"), oth_age = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>% 
...
#   fmt_number(columns = "percentage", decimals = 2) %>% 
#   tab_header(title = md(Title)) %>% 
#   cols_label(answer = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>% 
#   summary_rows(
#     column = c(participants,percentage),
#     fns= list(
#       Sum= ~sum(.)
...
dt_cig_frequency_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(n= md("**Number of Participants**"), often = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>% 
  # summary_rows(
  #   column = c(n,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
dtALC2_summary %>%  gt::gt(rowname_col = "row_lab") %>%
  fmt_number(columns = "percentage", decimals = 2) %>%
...
# data_tib_all3$D_726847824 <- factor(data_tib_all3$D_726847824,levels=c("Less Than once per Month ", "Once a month","2-3 times per Month","1-2 times per week","3-4 times per Week","5-6 times per Week","Every Day", "Skipped this question"))
...
dtBEER1_summary %>%  gt::gt(rowname_col = "row_lab") %>%
  fmt_number(columns = "percentage", decimals = 2) %>%
...
data_tib_all3$DOB <- paste0(str_sub(data_all3$d_371067537_1,1,4), "-", str_sub(data_all3$d_371067537_1,5,6), "-", str_sub(data_all3$d_371067537_1,7,8)) 
...
dtWORK2019_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
  tab_header(title = md("WORK2019: Work for Pay in 2019")) %>% 
  cols_label(n= md("**Number of Participants**"), meds = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>% 
  # summary_rows(
  #   column = c(n,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
dtWORKRES_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
...
dtPREWORK_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
...
PREWORKRES= dt_PREWORKRES %>%  mutate(meds= case_when(D_980007418==353358909 ~ "Yes",
...
dtPREWORKRES_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
...
Mod4$Country <- Mod4$D_121490150_D_831127170
  ...
  home1skip= home1skip %>%  mutate(Address1= case_when(!is.na(Street_Number) | !is.na(Street_Name) | !is.na(City) | !is.na(State) | !is.na(ZIP) |!is.na(Country)~"Provided Current Address",
...
dt_home1skip_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(n= md("**Number of Participants**"),Address1 = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>% 
  grand_summary_rows(columns=c(n, percentage),fns = ~sum(.,na.rm = T))|>
  tab_options(
      stub.font.weight = "bold"
    ) %>%
...
ssn_surv_summary_gt <- ssn_surv_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(n= md("**Number of Participants**"), started = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>% 
  # summary_rows(
  #   groups = everything(),
  #   column = c(n,percentage),
  #   fns= list(
...
# 
# dt_gavePssn_summary <- gavePssn_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
#   fmt_number(columns = "percentage", decimals = 2) %>% 
#   tab_header(title = md("Portion of SSN Given for Submitted SSN Surveys")) %>% 
#   cols_label(n= md("**Number of Participants**"), provided4 = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>% 
#   # summary_rows(
#   #   column = c(participants,percentage),
#   #   fns= list(
#   #     Sum= ~sum(.)
...
# 
# dt_gaveFssn_summary %>%  gt::gt() %>%  
#   fmt_number(columns = "percentage", decimals = 2) %>% 
#   tab_header(title = md("Full SSN Given of Those Who Started or Submitted the SSN Suvery")) %>% 
#   cols_label(n= md("**Number of Participants**"), providedall = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>% 
#   # summary_rows(
#   #   column = c(participants,percentage),
#   #   fns= list(
#   #     Sum= ~sum(.)
...
dt_all_ages_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(n= md("**Number of Participants**"), age = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>% 
  # summary_rows(
  #   column = c(participants,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
dt_sibcanc1_summary %>%  gt::gt(rowname_col = "row_lab") %>%  ## why is this not a proper gt_tbl??
  ...
  cols_label(status = md("**Answer**"), n = md("**Number of Participants**"), percentage = md("**Percentage of <br> Participants**")) %>%
    # summary_rows(
    # column = c(n,percentage),
    # fns= list(
    #   Sum= ~sum(.)
...
sibling_bd_canc <- dt_body_sibling_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(bodyp = md("**Answer**"), percentage = md("**Percentage of <br> Participants**")) %>% 
  # summary_rows(
  #   column = c(n,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
dt_curr_bus_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(n= md("**Number of Participants**"), category = md("**Answer**"), percentage = md("**%**")) %>% 
  # summary_rows( groups=F,
  #   column = c(n,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
